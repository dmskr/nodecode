...
exports.index = (req, res, next) ->
  req.locals =
    cursor: Code.db.users.find({}).sort(createdAt: -1)
    tab: "all"
...
exports.blocked = (req, res, next) ->
  req.locals =
    cursor: Code.db.users.find(access: false).sort(createdAt: -1)
    tab: "blocked"
...
exports.search = (req, res, next) ->
  Code.db.keywords.toConditions req.query.term, (err, conditions) ->
    return next(err)  if err
    conditions ||= {}
    req.locals =
      cursor: Code.db.users.find(conditions).sort(visitedAt: -1)
      tab: "search"
      term: req.query.term
...
exports.renderHtml = (req, res, next) ->
  ...
  , (err, results) ->
    return next(err)  if err
    res.render Code.root + "/app/users/admin/index.jade", Object.merge(results,
      page: page
...
exports.export = (req, res, next) ->
  req.locals = cursor: Code.db.users.find().sort(createdAt: -1)
  exports.renderCsv req, res, next

exports.show = (req, res, next) ->
  Code.db.users.findById req.params.id, (err, user) ->
    return next(err)  if err
    try
      user.emailmd5 = crypto.createHash("md5").update((user and user.email) or "").digest("hex").toString()
      res.render Code.root + "/app/users/admin/form.jade",
        man: user
    catch err
      next err
...
exports.autocomplete = (req, res, next) ->
  Code.db.users.autocomplete req.params.term or req.query.term, (err, users) ->
    return next(err) if err
    res.json "200",
      users: users
