...
describe "User Admin Controller", ->
  shouldBeLikeIndex = (options) ->
    ...
    it "should render proper view", (done) ->
      res.render = (template, options) ->
        template.should.eql "#{Code.root}/app/users/admin/index.jade"
        done()
      Code.apps.users.controller.admin[action](req, res, next)

    it "should not fail if no users found", (done) ->
      Code.db.users.remove (err) ->
  ...
  describe "index", ->
    ...
    it "should show recent first", (done) ->
      ...
      Code.apps.users.controller.admin.index(req, res, next)
  ...
  describe "active", ->
    ...
    beforeEach (done) ->
      Code.db.users.find({}).toArray (err, users) ->
        return done(err) if(err)
        async.each users, (user, next) ->
          user.access = true
          user.confirmedAt = Date.create()
          user.password = 'some'
          Code.db.users.save user, (err) ->
            return done(err) if(err)
            next()
        , done
    ...
    it "should show recent first", (done) ->
      ...
      Code.apps.users.controller.admin.active(req, res, next)

    it "should only show active users", (done) ->
      Code.db.users.update { title: { $gt: 5 }}, { $set: { access: false }}, { multi: true }, (err) ->
        return done(err) if(err)
        res.render = (view, params) ->
  ...
  describe "blocked", ->
    ...
    beforeEach (done) ->
      Code.db.users.update({}, { $set: { access: false }}, { multi: true }, done)

    it "should render proper view", (done) ->
      res.render = (template, options) ->
        template.should.eql(Code.root + '/app/users/admin/index.jade')
        done()
      Code.apps.users.controller.admin.blocked(req, res, next)
    ...
    it "should only show blocked users", (done) ->
      Code.db.users.update { title: { $gt: 5 }}, { $set: { access: true }}, { multi: true }, (err) ->
        return done(err) if(err)
        res.render = (view, params) ->
          params.data.length.should.eql(6)
          params.total.should.eql(6)
  ...
  describe "search", ->
    ...
    it "should show recent first", (done) ->
      ...
      Code.apps.users.controller.admin.search(req, res, next)
  ...
  describe "Show", ->
    user2show = null
    beforeEach (done) ->
      Code.db.users.save { email: 'dmitry@skrinnik.com' }, (err, user) ->
        user2show = user
    ...
    it "should select that concrete user", (done) ->
      res.render = (view, params) ->
        view.should.equal(Code.root + "/app/users/admin/form.jade")
        params.man._id.should.eql(user2show._id)
        done()
      Code.apps.users.controller.admin.show(req, res)
  ...
  describe 'autocomplete', ->
    autocomplete = null
    beforeEach ->
      autocomplete = Code.db.users.autocomplete
    afterEach ->
      Code.db.users.autocomplete = autocomplete

    it 'should render all users from autocomplete to json', (done) ->
      Code.db.users.autocomplete = (text, callback) ->
        callback null, [{ username: 'Monkey', email: 'monkey@coder.com' }, { email: 'godzilla@house.com' }]

      res.json = (st, data) ->
        data.users.length.should.eql(2)
        done()
      Code.apps.users.controller.admin.autocomplete(req, res, next)
